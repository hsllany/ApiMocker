...
class TemplateParser {
    ...
    check() {
        if (this.rawData.hasOwnProperty('main_module')) {
            TemplateParser.checkMainModule(this.rawData["main_module"]);
        }

        if (this.rawData.hasOwnProperty("modules")) {
            TemplateParser.checkModules(this.rawData['modules']);
        }
    }
    ...
    static checkMainModule(mainModule) {
        //TODO
    }

    static checkModules(modules) {
        if (!modules instanceof Array) {
            throw 'modules must be an array';
        }

        for (let module in modules) {
            TemplateParser.checkSingleModule(module);
        }
    }

    static checkSingleModule(singleModule) {
        //TODO
    };
}
